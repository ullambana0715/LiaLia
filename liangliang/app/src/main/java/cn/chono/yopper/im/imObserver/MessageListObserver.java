        ...
        if (observable instanceof MessageEvent) {
            ...
            if (msg != null) {
                ...
                if (dto != null) {
                    ...
                    long pre_time = SharedprefUtil.getLong(ContextUtil.getContext(), YpSettings.chat_message_list_time, 0);
                    if (pre_time == 0) {

                        SharedprefUtil.saveLong(ContextUtil.getContext(), YpSettings.chat_message_list_time, System.currentTimeMillis());

                        RxBus.get().post("ReceiveMessageUpdateList", new CommonEvent(dto));

                    } else {
                        long cur_time = System.currentTimeMillis();
                        if (cur_time - pre_time >= 1000) {
                            SharedprefUtil.saveLong(ContextUtil.getContext(), YpSettings.chat_message_list_time, cur_time);
